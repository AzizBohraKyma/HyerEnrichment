...
def _bash(script: str, *extra: str) -> list[str]:
    if platform.system() == "Windows":
        ...
        return ["wsl", "-d", "Ubuntu", "-u", "root", "bash", wsl_script, *extra]
...
def main() -> int:
    ...
    for name, script, extra in [
        ...
        ("tier2_e2e", "e2e_tier2.sh", []),
        ("tier3_e2e", "e2e_tier3.sh", []),
        ("strict_e2e", "e2e_realworld_strict.sh", []),
        ("full_path_live", "e2e_full_path.sh", ["--live"]),
    ]:
        cmd = _bash(script, *extra)
        code, out = _run(cmd)
