...
- **Daily notes:** `memory/YYYY-MM-DD.md` — local-only, gitignored. Create `memory/` on disk if needed; never commit to the org repo. Raw logs of what happened.
...
- When someone says "remember this" → update local `memory/YYYY-MM-DD.md` (gitignored) or relevant file
...
**Track your checks** in local `memory/heartbeat-state.json` (gitignored; same `memory/` folder):
...
1. Read through recent local `memory/YYYY-MM-DD.md` files (gitignored; on disk only)
